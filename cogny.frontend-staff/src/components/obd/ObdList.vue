<template>
  <div class="card card-table p-2">
    <div class="card-header">
      <span class="font-weight-bold mx-0"> OBD 단말기 목록 </span>
    </div>
    <div class="card-body">
      <b-container class="mw-100 dataTables_wrapper container-fluid dt-bootstrap4 p-0">
        <b-row class="mb-3">
          <b-col cols="12" class="text-right">
            <button class="btn btn-rounded btn-space btn-primary mr-4" @click="newObd">
              <i class="icon mdi material-icons">phonelink_setup</i> OBD 단말기 등록
            </button>
          </b-col>
        </b-row>
        <b-row class="px-4">
          <b-col cols="12" md="4" class="partner-input py-1" id="popupPartnerContainer">
              <b-input-group>
                <b-form-input v-model.trim="companyName" readonly></b-form-input>
                <b-btn id="popoverSelectPartner" variant="primary">제휴사 선택</b-btn>
                <b-btn
                    class="ml-1"
                    variant="primary"
                    @click="onSelectAllPartner()"
                    v-b-tooltip.hover title="전체 제휴사에서 보기">전체 보기</b-btn>
                <b-popover container="popupPartnerContainer" :show.sync="showPopupPartner" :placement="'bottomleft'" target="popoverSelectPartner">
                  <PopupPartnerList/>
                </b-popover>
              </b-input-group>
          </b-col>
          <b-col cols="12" md="5" class="py-1">
            <div class="input-group input-search">
              <b-form-input type="text" class="form-control" placeholder="OBD 단말기 검색" 
                            v-model="obdsPage.query" 
                            @keyup.enter.native="search()"
                            />
              <span class="input-group-btn">
                <button class="btn btn-secondary" @click="search()">
                  <i class="icon mdi mdi-search"></i>
                </button>
              </span>
            </div>
          </b-col>
          <b-col cols="12" md="3" class="text-right py-1">
            <span class="d-inline-flex">페이지당 </span>
            <div class="d-inline-flex">
              <b-form-select v-model="obdsPage.scale" :options="scaleOptions" @input="search"/>
            </div>
            <span class="d-inline-flex">개</span>
          </b-col>
        </b-row>
        <b-row class="be-datatable-body mt-4 mx-1">
          <b-col>
            <b-table striped hover :fields="obdFields" :items="obdsPage.contents" class="border-top border-bottom">
              <template slot="vehicleSlot" slot-scope="row">
                <div> {{ row.item.vehicle ? row.item.vehicle.licenseNo : '-'}}</div>
              </template>
              <template slot="partnerSlot" slot-scope="row">
                <div> {{ row.item.partner ? row.item.partner.companyName : '-'}}</div>
              </template>
              <template slot="manager" slot-scope="row">
                <div>
                  <button type="button" class="btn btn-secondary" @click="showInfo(row.item)" v-b-tooltip.hover title="OBD 단말기 정보수정"><i class="icon mdi mdi-edit"/></button>
                  <button type="button" class="btn btn-secondary" @click="onDelete(row.item)" v-b-tooltip.hover title="삭제"><i class="icon mdi mdi-delete"/></button>
                </div>
              </template>
            </b-table>
          </b-col>
        </b-row>
        <b-row class="be-datatable-footer mx-4">
          <b-col cols="5">
            <div class="dataTables_info" id="table1_info" role="status" aria-live="polite"> {{ obdsPage.page }} / {{ obdsPage.totalPages }} 페이지, 총 : {{ obdsPage.total }}개</div>
          </b-col>
          <b-col cols="7">
            <div class="dataTables_paginate paging_simple_numbers" id="table1_paginate">
              <b-pagination align="right" size="md" 
                            :total-rows="obdsPage.total" 
                            v-model="obdsPage.page" 
                            :per-page="obdsPage.scale" 
                            @input="movePage" />
            </div>
          </b-col>
        </b-row>
      </b-container>
    </div>
    <div>
      <b-modal ref="deleteYnModal" @ok="onDeleteOk" @hide="onHideDelete">
        <div class="text-center font-enlarge">
          OBD '<span class="text-danger font-weight-bold">{{ obdToBeDeleted ? obdToBeDeleted.obdSerial : null }}</span>'의 <br/> 정보를 삭제하시겠습니까? 
        </div>
      </b-modal>
    </div>
  </div>
</template>

<script>
import { mapGetters } from 'vuex'
import cf from '../../utils/customFilters'
import { eventBus } from '../common/EventBus.vue'
import PopupPartnerList from '../common/popup/PopupPartnerList.vue'

export default {
  name: 'ObdList',
  components: {
    PopupPartnerList
  }, 
  computed: mapGetters({ 
    obdsPage: 'getObdsPage'
  }),
  data: function(){
    return {
      companyName: null,
      showPopupPartner: false,
      params: {},
      scaleOptions: [
        { value: 15, text: 15 },
        { value: 30, text: 30 },
        { value: 50, text: 50 },
        { value: 100, text: 100 }
      ],
      obdFields: [
        { key: 'obdDeviceNo', label: '번호', class: 'text-center width-10' },
        { key: 'partnerSlot', label: '회사명', class: 'text-center width-20' },
        { key: 'obdSerial', label: 'OBD 단말기 고유번호', class: 'text-center width-20' },
        { key: 'vehicleSlot', label: 'OBD 설치 차량번호', class: 'text-center width-20' },
        { key: 'statusName', label: '설치여부', class: 'text-center width-10' },
        { key: 'regDate', label: '설치일', formatter: 'formatDate', class: 'text-center width-10' },
        { key: 'manager', label: '수정/삭제', class: 'text-center width-10' },
      ],
      obdToBeDeleted: null
    }
  },

  methods: {
    search() {
      this.params.query = this.obdsPage.query
      this.params.scale = this.obdsPage.scale
      this.params.page = 1
      this.$store.dispatch('loadObdsPage', this.params)
    },
    movePage() {
      this.params.page = this.obdsPage.page
      this.$store.dispatch('loadObdsPage', this.params)
    },
    newObd() {
      this.$router.push({ name: 'obdNew' })
    },
    showInfo(obd) {
      this.$router.push({ name: 'obdDetail', params: { obd : obd } })
    },
    onDelete(obd) {
      this.obdToBeDeleted = obd
      this.$refs.deleteYnModal.show()
    },
    onDeleteOk() {
      this.$store.dispatch('deleteObd', this.obdToBeDeleted.obdDeviceNo)
    },
    onHideDelete() {
      this.obdToBeDeleted = null
    },
    onSelectAllPartner() {
      this.params.partnerNo = null
      this.companyName = null
      this.$store.dispatch('loadObdsPage', this.params)
    },
    keydownHandler(event) {
      if (event.which === 13) {
        this.search()
      }
    },    
    formatDate: function(date) {
      return cf.formatDate(date)
    }
  },

  created() {
    console.log('created obd/ObdList')
    this.$store.dispatch('loadObdsPage', this.params)

    let that = this
    eventBus.$on('onSelectPopupPartner', function(partner){
      that.params.partnerNo = partner.partnerNo
      that.companyName = partner.companyName
      that.showPopupPartner = false;
      that.$store.dispatch('loadObdsPage', that.params)
    })

    eventBus.$on('onClosePopupPartnerList', function() {
      that.showPopupPartner = false
    })
  },
  mounted() {
    this.params = {
      page: this.obdsPage.page,
      scale: this.obdsPage.scale,
      query: this.obdsPage.query
    }
  } 
}
</script>
<style>
  .width-10 { width: 10% }
  .width-15 { width: 15% }
  .width-25 { width: 25% }
  .partner-input .popover {
    max-width: 600px;
}</style>