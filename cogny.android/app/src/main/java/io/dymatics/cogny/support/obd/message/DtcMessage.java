 ...
 */
public class DtcMessage extends SequentialMessage{
    @Getter private int emsCount;           // OFFSET + 1
    @Getter private int absCount;           // OFFSET + 92

    @Getter private List<String> dtcRaws;

    @Override
    protected void doPopulate() {
        parseSequence();

        int emsCountIndex = OFFSET + 1;
        int absCountIndex = OFFSET + 92;

        emsCount = toSignedInt(bytes.get(emsCountIndex));
        absCount = toSignedInt(bytes.get(absCountIndex));

        dtcRaws = new ArrayList<>();
        populate(emsCount, emsCountIndex + 1);  // EMS : OFFSET + 2,3,4 ~ 89,90,91
        populate(absCount, absCountIndex + 1);  // ABS : OFFSET + 93,94,95 ~ 180,181,182
    }

    private void populate(int count, int fromIndex) {
        for (int i = 0; i<count; i++) {
            int from = fromIndex + (i * 3);
            dtcRaws.add(String.format("%s,%s,%02X%02X,%02X", seq, time, bytes.get(from), bytes.get(from + 1), bytes.get(from + 2)));
        }
    }

    @Override
    protected String[] getStringArray() {
        return dtcRaws.toArray(new String[dtcRaws.size()]);
    }

    public boolean hasDtc() {
        return emsCount + absCount > 0;
    }
}
